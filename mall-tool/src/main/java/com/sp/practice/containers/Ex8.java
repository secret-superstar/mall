package com.sp.practice.containers;// containers/Ex8.java
...
class SList2<E> {
    private Link2<E> headLink = new Link2<E>(null);

    Slistiterator2<E> iterator() {
        return new Slistiterator2<E>(headLink);
    }

    public String toString() {
        if (headLink.next == null) return "SList: []";
        print("SList: [");
        Slistiterator2<E> it = this.iterator();
        StringBuilder s = new StringBuilder();
        while (it.hasNext()) {
            s.append(it.next() + (it.hasNext() ? ", " : ""));
        }
...
class Slistiterator2<E> {

    Link2<E> current;

    Slistiterator2(Link2<E> link) {
        current = link;
    }
    ...
    public void insert(E e) {
        current.next = new Link2<E>(e, current.next);
        current = current.next;
    }
...
public class Ex8 {
    public static void main(String[] args) {
        ...
        SList2<String> sl2 = new SList2<String>();
        println(sl);
        ...
        Slistiterator2<String> sltier2 = sl2.iterator();
        sltier2.insert("good");
        sltier2.insert("good2");
        sltier2.remove();
        sltier2.remove();
